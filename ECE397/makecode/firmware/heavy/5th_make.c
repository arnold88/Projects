...
while ( true ) // program starts!
{
  ...
  murmur_length = 0;
	   ...
	   Serial_Write( wait2, strlen( (char*)wait2 ), 1000 );
        ...
        if (DigitalIn_GetValue (4) == 1) default_mode();
	...
	// Display the heart detail
	 ...
	 // Display all other details ... entering final run
		...
		if ( (murmur_length < 10) || (murmur_flag == 0) )
	    {
		Serial_Write( no_murmur, strlen( (char*)no_murmur), 1000 );
	    }
	    else if (murmur_length >70)
	    {
	   ...
	   else if (murmur_length < 60)
	   {
		Serial_Write( early, strlen( (char*)early) , 1000 );
	   }
	   else if (murmur_length < 200)
	   {
...
void default_mode()
{
       int heart_rate[] = {7183,7210,7074,6484,6744, 6845, 8439, 8727, 9185, 7736, 7236, 6936};
	   int periodx [] = {0,0,0,1,1,1,0,0,0,2,2,2};
	   int lengthx [] = {0,0,0,3,2,1,0,0,0,2,2,2};
       int counter = 0;

       Serial_SetChar (12);
	   Sleep(10);

	   while (DigitalIn_GetValue(4) == 1) // release button
	   {
    ...
    while (true)
	{
	   ...
	   Serial_SetChar (12);
	    ...
	    if (DigitalIn_GetValue(4) == 1) //quit default
	    {
		    Serial_SetChar (147);
	        Serial_SetChar (82);
            while (DigitalIn_GetValue(4) == 1);
			return;
	    }

	   } // end while not on recording
	   ...
	   // Display the heart detail
		 ...
		 Serial_SetChar ( ((heart_rate[counter]) / 1000) + 48 );
		 Serial_SetChar ( ( (heart_rate[counter]) / 100) - ( ((heart_rate[counter]) / 1000)*10)  + 48 );
		 Serial_SetChar (46);
		 Serial_SetChar ( ((heart_rate[counter]%100) / 10) + 48  );
		 Serial_SetChar ( (heart_rate[counter] % 10) + 48  );

         Serial_SetChar(13);

	 // Display all other details ... entering final run

        if (periodx[counter] == 0)
		{
            Serial_Write( no_murmur, strlen( (char*)no_murmur ), 1000 );
		}
		else
	   ...
	   if (lengthx[counter]==0)
	   {
	   Serial_Write( no_murmur, strlen( (char*)no_murmur), 1000 );
       }
	   else if (lengthx[counter]==1)
	   {
		Serial_Write( early, strlen( (char*)early) , 1000 );
	   }
	   else if (lengthx[counter]==2)
	   {
		Serial_Write( late, strlen( (char*)late), 1000 );
	   }
	   else
