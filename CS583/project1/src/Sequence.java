...
public class Sequence {
	...
	public boolean isSubsequenceOf(Sequence s) {
		if (_length > s.getLength())
			return false;
		if (this.getSize() > s.getSize())
			return false;
		int lengthDifference = s.getLength() - _length;
		for (int i = 0; i < _sequence.size(); i++) {
			for (int j = 0; j < _sequence.get(i).size(); j++) {
				if (s.getItemset(i).size() < _sequence.get(i).size())
					return false;
				if (!s.getItemset(i).isInSet(_sequence.get(i).getItem(j)))
					return false;
				if (s.getLength() - i > lengthDifference)
					return false;
				else
					continue;
			}
