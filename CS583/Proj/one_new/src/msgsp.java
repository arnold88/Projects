    ...
    try{
        ...
        while (DIS.available() != 0)
	    {
     	...
     	// read in MIS value or SDC value
     	  ...
     	  {
     		 int j = line.indexOf('=');
        	 //MIS.add( BigDecimal.valueOf(Double.valueOf(line.substring(j+1, line.length()))));
     		
     		 double T = Double.valueOf(line.substring(j+1, line.length()));
...
//       }// end test output  --- comment this entire section in real mode
     ...
     {
    	 //double temp2 =  BigDecimal.valueOf(L.get(x).getCount()).divide(BigDecimal.valueOf(total_cust_count), 25, RoundingMode.HALF_DOWN).doubleValue() ;
    	 
    	 double temp2 =   (double) L.get(x).getCount()  / (double) total_item_count;
...
//    	   countSequences(dbase, temp_candidate, total_cust_count);
    	 ...
    	 else
    	 {
    	   ...
    	   // temp_candidate = level2CandidateGen(L,flas dbase, SDC, total_cust_count);
...
//		System.out.println(r_1.toString() + " reverse of " + s_1.toString());
//		System.out.println(r_2.toString() + " reverse of " + s_2.toString());
		ItemComp comp = new ItemComp();
		if (r_1.getItemset( r_1.getSize() - 1   ).getSize() == 1) { // the last item in s_1 is a separate element
			candidate_b.addItemset(r_1.getItemset( r_1.getSize() - 1  )); // that itemset is appended to s_2 for a candidate
			...
			if (r_2.getLength() == 2 && r_2.getSize() == 2 && comp.compare(r_1.getLast(), r_2.getLast()) < 0) { // the length of s_2 is 2 and size of s_2 is 1 AND last item of s_1 > last item of s_2
				...
				candidate_2.addItem(r_1.getLast(), candidate_2.getSize() - 1); // add last item of s_1 to end of s_2 to form candidate
		...
		
		{
			...
			candidate_b.getItemset(candidate_b.getSize() - 1).addItem(r_1.getLast()); // last item of s_1 is added to end of last item of s_2
 ...
 /*   public static int [] makeCount(ArrayList<Item> items, ArrayList<Sequence> DB)
    ...
    public static Itemset createItemSet(HashMap<Item, Boolean> items, String itemSetString, ArrayList<Double> MIS)
    {
        ...
        while (i < itemSetString.length())
        {
			...
			Item item = new Item(parsedName, MIS.get(Integer.parseInt(parsedName) - 1).doubleValue());
