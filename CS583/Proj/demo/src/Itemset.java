...
//	public boolean isInSDC(double sdc, int n, HashMap<Item, Double> supports) {
//		if (_itemset.size() > 1) {
//			for (int i = 0; i < _itemset.size(); i++) {
//				for (int j = 0; j < _itemset.size(); j++) {
//					if (Math.abs(_itemset.get(i).getSup(supports) - _itemset.get(j).getSup(supports)) > sdc) {
//						System.out.println(_itemset.get(i).toString() + " Support = " + _itemset.get(i).getSup(supports));
//						System.out.println(_itemset.get(j).toString() + " Support = " + _itemset.get(j).getSup(supports));
//						return false;
//					}
//				}
//			}
...
//	}
	...
	public boolean addItem(Item e) {
		if (this.isInSet(e))
			return false;
		_itemset.add(e);
		if (e.getMinsup().compareTo(_minItem.getMinsup()) < 0) {
			_minItem = e;
	...
	public void setMin() {
		if (_itemset.size() != 0) {
			...
			_minIndex = 0;
			for (int i = 0; i < _itemset.size(); i++) {
				if (_itemset.get(i).getMinsup().compareTo(_minItem.getMinsup()) < 0) {
