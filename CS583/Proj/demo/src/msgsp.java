...
public class msgsp {
   ...
   //static HashMap<Item, BigDecimal> supports = new HashMap<Item, BigDecimal>();
   static HashMap<Item, Double> supports = new HashMap<Item, Double>();
	...
	public static void main(String[] args) {
		...
		String Data = "C:\\data2.txt";
		String Param = "C:\\para2-1.txt";
	    String Output = "C:\\users\\user\\desktop\\myresult\\result2-1.txt";
	    ...
	    //ArrayList<BigDecimal> MIS = new ArrayList<BigDecimal>();
	    //ArrayList<Item> MIS = new ArrayList<Item>();
	    
	    
    	//BigDecimal SDC = BigDecimal.valueOf(0.0d);
	    double SDC = 0.0;
	    HashMap<String, Double> MIS = new HashMap<String, Double>();
    ...
    try{
        ...
        while (DIS.available() != 0)
	    {
     	...
     	// read in MIS value or SDC value
     	  ...
     	  {
     		 int j = line.indexOf('=');
//     		 SDC = BigDecimal.valueOf(Double.parseDouble(line.substring(j+1, line.length())));
//     		 SDC.setScale(3);
//     		 double tempSDC = double.valueOf(Double.valueOf(line.substring(j+1, line.length())));
//     		 SDC = tempSDC.setScale(3, double.ROUND_HALF_UP);
     	  ...
     	  {
     		 int j = line.indexOf('=');
     		 //double tempMIS = Double.valueOf(line.substring(j+1, line.length())));
     		 
//        	 BigDecimal mis = BigDecimal.valueOf(Double.valueOf(line.substring(j+1, line.length())));
//        	 mis.setScale(3, BigDecimal.ROUND_HALF_UP);
     		 Double mis = Double.valueOf(line.substring(j+1, line.length()));
     		int x = line.indexOf('(');
     		int y = line.indexOf(')');
     		String z = (line.substring(x+1,y));
     		 
     		 MIS.put(z, mis);
     		//MIS.add(  Math.round(Math.abs(Double.valueOf(line.substring(j+1, line.length()))  * 1000.00)) / 1000.00     );
...
//        double [][] MIS_diff = new double[MIS.size()][MIS.size()];
//        
//        //MIS difference look-up array
//       
...
//                 
//                     
//                  MIS_diff[i][j] = Math.round( Math.abs(  (MIS.get(i) - MIS.get(j) ) ) * 1000.00  ) / 1000.00 ;
// 
//               }
//           }
...
//        }
        ...
        Iterator it = MIS.entrySet().iterator();
...
//        }
        ...
        while (it.hasNext()) {
        	...
        	Item item = new Item( (String)me.getKey(), (double)me.getValue());
        	
        	
        	System.out.println((String)me.getKey() + " <-> " + (double)me.getValue());
...
//       }// end test output  --- comment this entire section in real mode
     ...
     for (Item i : items.keySet())
    	 map.add(i);
     L = L_Generation(map,total_cust_count, supports);
     ...
     {
//    	 BigDecimal temp2 = BigDecimal.valueOf(L.get(x).getCount()).divide(BigDecimal.valueOf(total_cust_count), 3, BigDecimal.ROUND_HALF_UP);
    	 Double temp2 = L.get(x).getCount() * 1.0 / total_cust_count;
    	 
    	 //int temp_index = Integer.parseInt(L.get(x).getName()) -1;
    	 
    	 //if (temp2.compareTo(MIS.get(temp_index)) >= 0) // single-item sequence will be included in F_one
    	 if (temp2 >= (Double) MIS.get(L.get(x).getName()))
    	 {
...
//     System.out.println("total size L:"+L.size());
    ...
    // Set the for loop to end at some arbitrary high value
     ...
     for (int k=2 ; k <= MAX_LIMIT*10 ; k++ )  // this is line 4 in p.46 in the book
     {
    	 ...
    	 if (k==2) // line 5 
    	 {
    	   //temp_candidate = level2CandidateGen(L, dbase, SDC, total_cust_count, supports, MIS_diff);
    		 temp_candidate = level2CandidateGen(L, dbase, SDC, total_cust_count, supports); 
//    	   countSequences(dbase, temp_candidate, total_cust_count);
    	 ...
    	 for (Sequence s : temp_candidate)
    		 System.out.println(s.toString() + " : " + s.getCount() + " : " + s.getSup() + " : " + s.getMinsup() + " : " + s.isInSDC(SDC, total_cust_count, supports));
...
//   			 }
//   		 }
    	 temp_f = pruneCandidatesSDC(temp_candidate, dbase, SDC, total_cust_count, supports);
    	 //temp_f = temp_candidate;
    	 
    	 //if (k == 3) break; //TODO REMOVE BEFORE SUBMISSION
  ...
  // either return or output function goes here
    ...
    System.out.println("Results:");
 	...
 	for (int a=0;a<F_one.size();a++)
 	{
 		  ...
 		  output.print(F_one.get(a).toString() + "\t");
	      output.print("\tCount =\t" + F_one.get(a).getCount());
		  output.println("");
 	}
     ...
     for (int b=0; b<F_higher.size(); b++)
     {
    	 ...
    	 output.println("The number of length "+length+"-sequential patterns is "+ (F_higher.get(b).size()));
    	 output.println("Pattern:");
    	      ...
    	      for (int c=0;c<F_higher.get(b).size(); c++)
    	      {
    	    	  System.out.print(F_higher.get(b).get(c).toString() + "\t");
    	 		  ...
    	 		  output.print(F_higher.get(b).get(c).toString() + "\t");
   	 	      	  output.print("\tCount =\t" + F_higher.get(b).get(c).getCount());
   	 	      	  output.println("");
	 ...
	 */
	//public static ArrayList<Sequence> level2CandidateGen(ArrayList<Item> L, ArrayList<Sequence> dbase, BigDecimal sdc, int n, HashMap<Item, BigDecimal> supports) {
	public static ArrayList<Sequence> level2CandidateGen(ArrayList<Item> L, ArrayList<Sequence> dbase, double sdc, int n, HashMap<Item, Double> supports) {
		ArrayList<Sequence> candidates = new ArrayList<Sequence>();
		ItemComp comp = new ItemComp();
		//BigDecimal minDiff = BigDecimal.valueOf(0.001d);
		
		for (int i = 0; i < L.size(); i++) {
			if ((L.get(i).getSup(supports) >= L.get(i).getMinsup())) {
			//if ((L.get(i).getCount() >= Math.floor(L.get(i).getMinsup() * n))) {
			//if (L.get(i).getSup(supports).compareTo(L.get(i).getMinsup()) >= 0) {
			//if (L.get(i).getCount() >= L.get(i).getMinsup().multiply(BigDecimal.valueOf(n)).intValue()) {
				//	|| (BigDecimal.valueOf(L.get(i).getSup(supports)).subtract(BigDecimal.valueOf(L.get(i).getMinsup())).abs().compareTo(minDiff)) <= 0) {
				for (int j = i; j < L.size(); j++) {
					if ((L.get(j).getSup(supports) >= L.get(i).getMinsup())) { 
					//if ((L.get(j).getCount() >= Math.floor(L.get(i).getMinsup() * n))) {
					//if (L.get(j).getSup(supports).compareTo(L.get(i).getMinsup()) >= 0) {
					//if (L.get(j).getCount() >= L.get(i).getMinsup().multiply(BigDecimal.valueOf(n)).intValue()) {
						//	|| (BigDecimal.valueOf(L.get(j).getSup(supports)).subtract(BigDecimal.valueOf(L.get(i).getMinsup())).abs().compareTo(minDiff)) <= 0) {
						...
						l.addItem(L.get(i));
						h.addItem(L.get(j));
						//System.out.println(Math.abs(L.get(j).getSup() - L.get(i).getSup()) + " ?= " + sdc);
						if ((Math.abs(L.get(j).getSup(supports) - L.get(i).getSup(supports)) <= sdc)) { 
						//		|| (BigDecimal.valueOf(L.get(j).getSup(supports)).subtract(BigDecimal.valueOf(L.get(i).getMinsup())).abs().subtract(BigDecimal.valueOf(sdc)).abs().compareTo(minDiff)) <= 0) {
						//if (MIS_diff[Integer.parseInt(L.get(j).getName()) - 1][Integer.parseInt(L.get(i).getName()) - 1] <= sdc) {
						//if (L.get(j).getSup(supports).subtract(L.get(i).getSup(supports)).abs().compareTo(sdc) <= 0) {
						//if (Math.abs(L.get(j).getCount() - L.get(i).getCount()) <= Math.ceil(sdc * n))  {
						//if (Math.abs(L.get(j).getCount() - L.get(i).getCount()) <= sdc.multiply(BigDecimal.valueOf(n)).intValue()) {
...
//			System.out.println(c.toString() + "\t:\t" + c.getFrequent());
	...
	//*******************************************************
	/*
	 * If the first item in s_1 is the minItem
	 */
	
	public static void minsupFirstGen(ArrayList<Sequence> candidates, Sequence s_1, Sequence s_2) {
		Sequence c_1 = s_1.copy(); Sequence c_2 = s_2.copy();
		ItemComp comp = new ItemComp();
		c_1.removeSecond(); c_2.removeLast();
//		System.out.println("Length : " + s_1.getLength() + "\tSize : " + s_1.getSize());
//		System.out.println("Length : " + s_2.getLength() + "\tSize : " + s_2.getSize());
//		System.out.println("s_1 Minsup = " + s_1.getMinsup() + "\ts_2 Minsup = " + s_2.getMinsup());
//		System.out.println(c_1.toString() + " + " + c_2.toString() + " : " + c_1.equals(c_2));
//		System.out.println(comp.compare(s_2.getLast(), s_1.getLast()));
		if (c_1.equals(c_2) && // (1)
				(s_2.getLast().getMinsup() > s_1.getFirst().getMinsup())) { // (2)
				//(s_2.getLast().getMinsup().compareTo(s_1.getLast().getMinsup()) > 0)) {
			Sequence candidate_a = s_1.copy();
			if (s_2.getItemset(s_2.getSize() - 1).getSize() == 1) { // last item {l} in s_2 is a separate element
				//if (candidate_a.addItemset(s_2.getItemset(s_2.getSize() - 1))) //System.out.println("A");
				candidate_a.addItemset(s_2.getItemset(s_2.getSize() - 1)); // adds single-Item Itemset {l} to the end of s_1 as a candidate
				//System.out.println(candidate_a.toString());
				...
				if ((s_1.getLength() == 2 && s_1.getSize() == 2) && // if s_1 has two Itemsets with one Item in each e.g., <{x}{y}> AND
					...
					//if (candidate_2.addItem(s_2.getLast(), s_1.getSize() - 1)) //System.out.println("B");
					candidate_2.addItem(s_2.getLast(), s_1.getSize() - 1);
					//System.out.println(candidate_2.toString());
					candidates.add(candidate_2);
				}
			}
			// (length of s_1 is 2 and size is 1 e.g., <{x, y}> AND last item in s_2 > last item in s_1) OR s_1 length is greater than 2
			else if (((s_1.getLength() == 2) && (s_1.getSize() == 1) && (comp.compare(s_2.getLast(), s_1.getLast()) > 0)) || (s_1.getLength() > 2)) { 
				//if (candidate_a.addItem(s_2.getLast(), candidate_a.getSize() - 1)) //System.out.println("C");
				candidate_a.addItem(s_2.getLast(), candidate_a.getSize() - 1);
				//System.out.println(candidate_a.toString());
				candidates.add(candidate_a);
			}	
		}
	...
	public static void minsupLastGen(ArrayList<Sequence> candidates, Sequence s_1, Sequence s_2) {
		Sequence r_1 = s_1.reverse(); Sequence r_2 = s_2.reverse(); Sequence candidate_b = r_2.copy(); 
		System.out.println("Joining:\t" + r_1.toString() + " and " + r_2.toString());
		//System.out.println(r_2.toString() + " reverse of " + s_2.toString());
		ItemComp comp = new ItemComp();
		if (r_1.getItemset(0).getSize() == 1) { // the last item in s_1 is a separate element
			if (candidate_b.addItemset(r_1.getItemset(r_1.getSize() - 1))) //System.out.println("A"); // that itemset is appended to s_2 for a candidate
			//System.out.println(candidate_b.toString());
			//candidate_b.addItemset(r_1.getItemset(r_1.getSize() - 1));
			//System.out.println(candidate_b.toString() + " : " + candidate_b.getLength() + " : A");
				candidates.add(candidate_b.reverse());
			if (r_2.getLength() == 2 && r_2.getSize() == 2 && comp.compare(r_1.getLast(), r_2.getLast()) < 0) { // the length of s_2 is 2 and size of s_2 is 1 AND last item of s_1 > last item of s_2
				Sequence candidate_2 = r_2.copy();
				if (candidate_2.addItem(r_1.getLast(), candidate_2.getSize() - 1)) //System.out.println("B"); // add last item of s_1 to end of s_2 to form candidate
				//System.out.println(candidate_2.reverse());
				//candidate_2.addItem(r_1.getLast(), candidate_2.getSize() - 1);
				//System.out.println(candidate_2.toString() + " : " + candidate_2.getLength() + " : B");
					candidates.add(candidate_2.reverse());
			}
		}
		else if ((r_2.getLength() == 2 && r_2.getSize() == 1 && comp.compare(r_2.getLast(), r_1.getLast()) < 0) || // the length of s_2 is 2 and size of s_2 is 1 AND last item of s_1 > last item of s_2 s_2 is 1
				(r_2.getLength() > 2)) {
			if (candidate_b.addItem(r_1.getLast(), candidate_b.getSize() - 1)) //System.out.println("C"); // last item of s_1 is added to end of last item of s_2
			//System.out.println(candidate_b.reverse());
			//candidate_b.getItemset(candidate_b.getSize() - 1).addItem(r_1.getLast());
			//System.out.println(candidate_b.toString() + " : " + candidate_b.getLength() + " : C");
				candidates.add(candidate_b.reverse());
		}
	}
	...
	public static void elseGen(ArrayList<Sequence> candidates, Sequence s_1, Sequence s_2) {
		Sequence c_1 = s_1.copy(); Sequence c_2 = s_2.copy();
		if (c_1.removeFirst().equals(c_2.removeLast())) { // if the s_1 minus first = s_2 minus last
			...
			if (s_2.getItemset(s_2.getSize() - 1).getSize() == 1) { // if the last item in s_2 was a separate element
				if (candidate_a.addItemset(s_2.getItemset(s_2.getSize() - 1))) // last itemset is appended to s_1
				//candidate_a.addItemset(s_2.getItemset(s_2.getSize() - 1));
					candidates.add(candidate_a);
			} else { // otherwise
				if (candidate_a.getItemset(candidate_a.getSize() - 1).addItem(s_2.getLast())) // last item of s_2 is added to end of last itemset of s_1
				//candidate_a.addItem(s_2.getLast(), candidate_a.getSize() - 1);
					candidates.add(candidate_a);
			}
		}
	...
	public static ArrayList<Sequence> MScandidateGen(ArrayList<Sequence> f, ArrayList<Sequence> dbase, int n) {
		...
		for (int i = 0; i < f.size(); i++) {
			for (int j = 0; j < f.size(); j++) { // line 1
				...
				//System.out.println("Joining " + s_1.toString() + " and " + s_2.toString());
				if (s_1.getFirst().equals(s_1.getMin())) { //line 2
					//System.out.println("MinItem is first in s_1");
					minsupFirstGen(candidates, s_1, s_2);
				}
				else if (s_2.getLast().equals(s_2.getMin())) { // line 3 if the MIS value of the last item in s_2 is the minsup of s_2
					System.out.println("MinItem is last in s_2");
					minsupLastGen(candidates, s_1, s_2);
				}
				else { // line 4
					//System.out.println("MinItem is in the middle of s_1 and s_2.");
					elseGen(candidates, s_1, s_2);
				}
			}
		}
		
		// Prune Step
		candidates = pruneMSGenCandidates(candidates, f, dbase, n);
		return candidates;
	}
	//**************************************************************
	
	public static ArrayList<Sequence> pruneMSGenCandidates(ArrayList<Sequence> candidates, ArrayList<Sequence> fk_1, ArrayList<Sequence> dbase, int n) {
		// Prune Step
		...
		for (Sequence c : candidates) {
			for (int j = 0; j < c.getSize(); j++) {
				for (int k = 0; k < c.getItemset(j).getSize(); k++) { 
					...
					temp.removeItem(j, k); // subsequence where item at j, k is removed
					temp.setCount(dbase, n);
					//System.out.println(temp.toString() + " : " + temp.getFrequent());
	...
	//public static ArrayList<Sequence> pruneCandidatesSDC(ArrayList<Sequence> candidates, ArrayList<Sequence> dbase, BigDecimal sdc, int n, HashMap<Item, BigDecimal> supports) {
	public static ArrayList<Sequence> pruneCandidatesSDC(ArrayList<Sequence> candidates, ArrayList<Sequence> dbase, Double sdc, int n, HashMap<Item, Double> supports) {
		ArrayList<Sequence> prunedCandidates = new ArrayList<Sequence>();
...
//					}
//				}
				if (c.isInSDC(sdc, n, supports)) prunedCandidates.add(c);
...
//	public static ArrayList<Item> L_Generation(ArrayList<Item> items, int total_count, HashMap<Item, BigDecimal> supports) {
	public static ArrayList<Item> L_Generation(ArrayList<Item> items, int total_count, HashMap<Item, Double> supports) {
...
//			System.out.println(sortedItem.get(x).getMinsup());
			...
			//BigDecimal support = BigDecimal.valueOf(i.getCount()).divide(BigDecimal.valueOf(total_count), 3, BigDecimal.ROUND_HALF_UP);
			Double support = i.getCount() * 1.0 / total_count;
...
//    public static Itemset createItemSet(HashMap<Item, Boolean> items, String itemSetString, ArrayList<BigDecimal> MIS)
	public static Itemset createItemSet(HashMap<Item, Boolean> items, String itemSetString, HashMap<String, Double> MIS)
    {
        ...
        while (i < itemSetString.length())
        {
            ...
            Item item = new Item(parsedName, (Double)MIS.get(parsedName)); //CHANGED
...
//    public static ArrayList<Sequence> createDB(HashMap<Item, Boolean> items, String Data, ArrayList <BigDecimal> MIS)
	public static ArrayList<Sequence> createDB(HashMap<Item, Boolean> items, String Data, HashMap<String, Double> MIS)
    {
        ...
        for (Item t: items.keySet())
        {
            System.out.println("item name:"+ t.getName() + "  count:  " + t.getCount());
        }
