...
public class Sequence {
	...
	public void setCount(ArrayList<Sequence> dbase, Integer n) {
		for (Sequence s : dbase) {
			if (this.isSubsequenceOf(s)) this.incCount();
		}
		  ...
		  BigDecimal Trans = new BigDecimal (n);
		 
		  Support = Count.divide(Trans, 8, BigDecimal.ROUND_FLOOR);
		
		 _support = Support.doubleValue();
		
		//System.out.println(_support);
		
		
		if (this.getSup() >= this.getMinsup()) _frequent = true;
		this.setLength();
		this.setMinsup();
	}
	...
	public boolean isSubsequenceOf(Sequence data) {
         ...
         for(i = 0; i < m; i++) {
             ...
             do {
                 ...
                 if (data.getItemset(j).contains(is)) {        //find an element in data which is the super set of current element in current sequence
                     j++;
                     break;
                 }
                 j++;
             ...
             if (i == m - 1 && j == n) result = data.getItemset(j - 1).contains(is);
         }
         return result;
	}
	
	// checks if the sequence is within the support difference constraint
	public boolean isInSDC(Double sdc, Integer n) {
		...
		for (int i = 0; i < allItems.size(); i++) {
			for (int j = 0; j < allItems.size(); j++) {
				if (Math.abs(allItems.get(i).getSup() - allItems.get(j).getSup()) > sdc)
					return false;
			}
		}
		return true;
...
//		}
	...
	public boolean addItem(Item e, int itemsetIndex) {
		...
		if (_minItemset == null || (_minItemset != null && (e.getMinsup() < _sequence.get(itemsetIndex).getMinItem().getMinsup()))) {
			_minItemset = indexed;
