...
static unsigned long bloom_m = 1 <<20;		//changed from 16 to 24

/*parameter n, the number of keys added to the bit vector*/
static unsigned long bloom_n = (1 <<13);

/* parameter k, the number of hash functions*/
static int bloom_k = 4;		//changes from 4 to 8

/*parameter TEST_LEN*/
static unsigned long TEST_LEN=(1<<17);
...
void bloom_opt_check(int argc, char **argv)
{
    ...
    if (argc == 1)
    {
     ...
     {
         fprintf(stderr, " Wrong input numbers...prog bloom_m bloom_n bloom_k test_len\n");
         exit(0);
     }    
}
...
static unsigned long bloom_hash0(md_addr_t pc, md_addr_t tpc, md_inst_t inst)
{
	...
	sprintf(str, "%x", pc);		//converting hex to string
	sprintf(str1, "%x", tpc);	//converting hex to string
	strcat(str,str1);		//concatinating two string
	...
	SHA1Result(&sha, Message_Digest);
     ...
     return compress32_16(Message_Digest[3],Message_Digest[2],Message_Digest[1],Message_Digest[0]); //(b3,b2,b1,b0);
}
...
static unsigned long bloom_hash1(md_addr_t pc, md_addr_t tpc, md_inst_t inst)
{
     ...
     */
     ...
     return compress32_16(Message_Digest[7],Message_Digest[6],Message_Digest[5],Message_Digest[4]);//(b3,b0,b1,b2);
}
...
static unsigned long bloom_hash2(md_addr_t pc, md_addr_t tpc, md_inst_t inst)
{
     ...
     */
     ...
     return compress32_16(Message_Digest[11],Message_Digest[10],Message_Digest[9],Message_Digest[8]);//(b0,b3,b1,b2);
}
...
static unsigned long bloom_hash3(md_addr_t pc, md_addr_t tpc, md_inst_t inst)
{
    ...
    */
    ...
    return compress32_16(Message_Digest[15],Message_Digest[14],Message_Digest[13],Message_Digest[12]);//(b3,b1,b0,b2);
}
...
static unsigned long bloom_hash4(md_addr_t pc, md_addr_t tpc, md_inst_t inst)
{
    ...
    return compress32_16(Message_Digest[19],Message_Digest[18],Message_Digest[17],Message_Digest[16]);//(b3,b1,b0,b2);
}
...
static unsigned long bloom_hash5(md_addr_t pc, md_addr_t tpc, md_inst_t inst)
{
	...
	*/	   
	...
	return compress32_16(Message_Digest[0],Message_Digest[4],Message_Digest[8],Message_Digest[12]);//(b3,b1,b0,b2);
}
...
static unsigned long bloom_hash6(md_addr_t pc, md_addr_t tpc, md_inst_t inst)
{
	...
	*/	                           
	...
	return compress32_16(Message_Digest[1],Message_Digest[5],Message_Digest[9],Message_Digest[13]);//(b3,b1,b0,b2);
}
...
static unsigned long bloom_hash7(md_addr_t pc, md_addr_t tpc, md_inst_t inst)
{
	...
	*/	                            
	...
	return compress32_16(Message_Digest[2],Message_Digest[6],Message_Digest[10],Message_Digest[14]);//(b3,b1,b0,b2);
}
...
void bloom_init()
{
    ...
    bloom_hash_funs[4] = &bloom_hash4;		//added newline for bloom_hash4
...
int MAIN_TEST(int argc, char **argv)
{
    ...
    output  =  check_open("resultings", "a");
    ...
    for (i=0; i < bloom_n; i++)
    {
       ...
       /* fprintf(stderr, "0x%x, 0x%x, 0x%x\n", rpc_a[i], rtpc_a[i], rinst_a[i]); */ //commented out
     }
     
     for (i=bloom_n-1; i >=0; i--)
    {
         ...
         else
	 {
	     ...
	     {
	//	fprintf(stderr, "False positive i: %d pc: 0x%x, tpc:0x%0x; inst:0x%x \n", i, rpc_b, rtpc_b, rinst_b);
		false_positive ++;
	     }
	 }	     
     }
     //fprintf(stderr, "false positive rate: %10.9f\n", (double) false_positive/TEST_LEN);
     fprintf(stderr, "occurrence %d (out of %d); \nfalse positive rate: %10.9f\n",
